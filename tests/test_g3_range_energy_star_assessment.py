...
from scripts.g3_range_energy_star_assessment import assess_record


def source_row(sku, candidates, plp="Y", badge=True, spec=True):
    return {
        "exact_sku": sku,
        "range_epa_pattern_candidates": candidates,
...
class RangeEnergyStarAssessmentTests(unittest.TestCase):
    def test_registered_electric_model_with_all_points_passes(self):
        row = source_row("RANGE1", [{"markets_raw": "United States, Canada"}])
    ...
    def test_non_us_only_match_is_not_a_us_registration(self):
        row = source_row("RANGE5", [{"markets_raw": "Canada"}], plp="N", badge=False, spec=False)
        result = assess_record(row)
        self.assertEqual(result["display_outcome"], "PASS")

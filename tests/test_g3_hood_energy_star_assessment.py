...
from scripts.g3_hood_energy_star_assessment import assess_record
...
class HoodEnergyStarAssessmentTests(unittest.TestCase):
    def test_registered_electric_model_with_all_points_passes(self):
        row = source_row("HOOD1", [{"markets_raw": "United States, Canada"}])
    ...
    def test_non_us_only_match_is_not_a_us_registration(self):
        row = source_row("HOOD5", [{"markets_raw": "Canada"}], plp="N", badge=False, spec=False)
        result = assess_record(row)
        self.assertEqual(result["display_outcome"], "PASS")

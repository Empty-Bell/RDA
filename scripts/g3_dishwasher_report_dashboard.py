...
def build(report_path,out):
 ...
 for x in r["rows"]:
  ...
  rows.append({"exact_sku":x["exact_sku"],"energy_star_publication":es,"energyguide_numeric":num["outcome"],"pdp_vs_energyguide_energy":num["comparisons"]["pdp_vs_energyguide_energy"],"energyguide_vs_epa_energy":num["comparisons"]["energyguide_vs_epa_energy"],"pdp_vs_epa_place_settings":num["comparisons"]["pdp_vs_epa_place_settings"],"finding_count":len(x["findings"])})
 ...
 (out/"index.html").write_text("""<!doctype html><meta charset=utf-8><title>Dishwasher regulatory audit</title><main><h1>Dishwasher regulatory audit</h1><pre id=s></pre><label>SKU <input id=q></label><table><thead><tr><th>SKU</th><th>Energy Star</th><th>PDP / Label energy</th><th>Label / EPA energy</th><th>PDP / EPA place settings</th><th>Findings</th></tr></thead><tbody id=t></tbody></table></main><script>Promise.all(['data/summary.json','data/report-data.json'].map(x=>fetch(x).then(x=>x.json()))).then(([s,d])=>{document.querySelector('#s').textContent=JSON.stringify(s,null,2);let draw=()=>t.innerHTML=d.filter(x=>x.exact_sku.includes(q.value.toUpperCase())).map(x=>`<tr><td>${x.exact_sku}</td><td>${x.energy_star_publication}</td><td>${x.pdp_vs_energyguide_energy}</td><td>${x.energyguide_vs_epa_energy}</td><td>${x.pdp_vs_epa_place_settings}</td><td>${x.finding_count}</td></tr>`).join('');q.oninput=draw;draw()})</script>""",encoding="utf-8")

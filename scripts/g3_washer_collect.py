"""Collect exact-SKU washer PDP facts from a successful source artifact."""
import argparse
...
CONTRACT = "G3_WASHER_EXACT_SKU_PDP_V1"
...
def load_population(recon_root, source_run_id):
    ...
    if recon.get("status") != "PASS" or str(recon.get("run_id")) != str(source_run_id):
        raise ValueError("Washer source artifact is not the requested successful run")
    if recon.get("scope") != "washer source contracts only":
        raise ValueError("Source artifact is not scoped to clothes washers")
    pages_by_offset = {}
    for observation in recon.get("observations", []):
        ...
        if digest != observation.get("fixture_sha256"):
            raise ValueError("Washer PF fixture hash mismatch")
        offset = int(request["startIndex"])
        page = json.loads(raw)
        if offset in pages_by_offset and pages_by_offset[offset][0] != page:
            raise ValueError("Washer PF offset changed within source artifact")
        pages_by_offset[offset] = (page, digest)
    if not pages_by_offset:
        raise ValueError("Washer source artifact has no PF pages")
        ...
        for group in page["searchResults"]:
            for variant in group["groupedProductList"]:
                ...
                products.append({"run_id": source_run_id, "exact_sku": sku, "listings": [{
                    "product_group": "washer", "source_family_id": group["group_id"],
...
def verify_identity(sku, final_url, snapshot, bridge):
    ...
    if parts.scheme != "https" or parts.hostname != "www.samsung.com" or "/us/" not in parts.path or not re.search(r"-sku-" + slug + r"/?$", parts.path.lower()):
        raise ValueError("Final washer PDP URL refers to another exact SKU")
    if snapshot.get("jsonld_parse_errors"):
        raise ValueError("Washer PDP JSON-LD parse failed")
    declarations = snapshot.get("product_jsonld")
    if not isinstance(declarations, list) or not declarations:
        raise ValueError("Washer PDP has no current Product JSON-LD identity")
    for declaration in declarations:
        ...
        if not identities or any(value != sku for value in identities):
            raise ValueError("Washer PDP JSON-LD does not match exact SKU")
    return pdp_facts(bridge, sku, family="washer")
        ...
        for product in products:
            ...
            def on_response(response):
                ...
                try:
                    if len(record["bridge_responses"]) > 8:
                        raise ValueError("Washer bridge response limit exceeded")
                    if response.status != 200 or "json" not in response.headers.get("content-type", "").lower():
                        raise ValueError("Washer bridge response unavailable or not JSON")
            ...
            try:
                ...
                if response is None or response.status != 200:
                    raise ValueError("Washer PDP HTTP request failed")
                if sku.lower() not in page.locator("body").inner_text().lower():
                    raise ValueError("Exact washer SKU is absent from rendered PDP text")
                candidates = []
                ...
                if len({candidate[0]["sha256"] for candidate in candidates}) != 1:
                    raise ValueError("Washer bridge changed during same-PDP collection")
                bridge_entry, facts = candidates[0]
                listing = product.get("source_claim_listing_raw")
                if not isinstance(listing, dict):
                    raise ValueError("Washer PF claim declaration is unavailable")
...
def coverage(products, results):
    ...
    if len(population) != len(products) or len(sku_list) != len(set(sku_list)) or not set(sku_list) <= population:
        raise ValueError("Washer PDP result identities are duplicate or outside population")
    ...
    if any(status not in ("VERIFIED_EXACT_IDENTITY", "FAILED") for status in by_sku.values()):
        raise ValueError("Washer PDP result status is invalid")
...
def main():
    ...
    report = {"contract": CONTRACT, "source_run_id": str(args.source_run_id),
              ...
              "captured_at": datetime.now(timezone.utc).isoformat(),
              "scope": "Washer exact-SKU PDP identity and source facts only; no combo routing, matching or assessment",
    ...
    if summary:
        with Path(summary).open("a", encoding="utf-8") as stream:
            stream.write("## Clothes washer exact-SKU PDP collection\n\n")

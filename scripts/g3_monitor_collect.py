"""Collect exact-SKU monitor PDP facts from a successful source artifact."""
import argparse
...
PLP_URL = "https://www.samsung.com/us/monitors/all-monitors/"
CONTRACT = "G3_MONITOR_EXACT_SKU_PDP_V1"
...
def load_population(recon_root, source_run_id):
    ...
    if (recon.get("status") not in ("PASS", "RUNNING") or not checks_pass
        ...
        raise ValueError(
            "Monitor source artifact is not the requested successful run: "
            f"status={recon.get('status')!r}, artifact_run_id={recon.get('run_id')!r}, "
            f"requested_run_id={source_run_id!r}"
        )
    if recon.get("scope") != "monitor source contracts only":
        raise ValueError("Source artifact is not scoped to monitors")
    pages_by_offset = {}
    for observation in recon.get("observations", []):
        ...
        if digest != observation.get("fixture_sha256"):
            raise ValueError("Monitor PF fixture hash mismatch")
        offset = int(request["startIndex"])
        page = json.loads(raw)
        if offset in pages_by_offset and pages_by_offset[offset][0] != page:
            raise ValueError("Monitor PF offset changed within source artifact")
        pages_by_offset[offset] = (page, digest)
    if not pages_by_offset:
        raise ValueError("Monitor source artifact has no PF pages")
    ...
    if len(canonical) != population["unique_exact_skus"]:
        raise ValueError("Monitor exact-SKU population cardinality changed")
...
def verify_identity(sku, final_url, snapshot, bridge):
    ...
    if parts.scheme != "https" or parts.hostname != "www.samsung.com" or "/us/" not in parts.path or not re.search(r"-sku-" + slug + r"/?$", parts.path.lower()):
        raise ValueError("Final monitor PDP URL refers to another exact SKU")
    if snapshot.get("jsonld_parse_errors"):
        raise ValueError("Monitor PDP JSON-LD parse failed")
    declarations = snapshot.get("product_jsonld")
    if not isinstance(declarations, list) or not declarations:
        raise ValueError("Monitor PDP has no current Product JSON-LD identity")
    for declaration in declarations:
        ...
        if not identities or any(value != sku for value in identities):
            raise ValueError("Monitor PDP JSON-LD does not match exact SKU")
    return pdp_facts(bridge, sku, family="monitor")
        ...
        for product in products:
            ...
            def on_response(response):
                ...
                try:
                    if len(record["bridge_responses"]) > 8:
                        raise ValueError("Monitor bridge response limit exceeded")
                    if response.status != 200 or "json" not in response.headers.get("content-type", "").lower():
                        raise ValueError("Monitor bridge response unavailable or not JSON")
            ...
            try:
                ...
                if response is None or response.status != 200:
                    raise ValueError("Monitor PDP HTTP request failed")
                if sku.lower() not in page.locator("body").inner_text().lower():
                    raise ValueError("Exact monitor SKU is absent from rendered PDP text")
                candidates = []
                ...
                if len({candidate[0]["sha256"] for candidate in candidates}) != 1:
                    raise ValueError("Monitor bridge changed during same-PDP collection")
                bridge_entry, facts = candidates[0]
                listing = product.get("source_claim_listing_raw")
                if not isinstance(listing, dict):
                    raise ValueError("Monitor PF claim declaration is unavailable")
...
def coverage(products, results):
    ...
    if len(population) != len(products) or len(sku_list) != len(set(sku_list)) or not set(sku_list) <= population:
        raise ValueError("Monitor PDP result identities are duplicate or outside population")
    ...
    if any(status not in ("VERIFIED_EXACT_IDENTITY", "FAILED") for status in by_sku.values()):
        raise ValueError("Monitor PDP result status is invalid")
...
def main():
    ...
    report = {"contract": CONTRACT, "source_run_id": str(args.source_run_id),
              ...
              "captured_at": datetime.now(timezone.utc).isoformat(),
              "scope": "Monitor exact-SKU PDP identity and source facts only; no combo routing, matching or assessment",
              "assessment": "NOT_EVALUATED", "population": population, "coverage": rows,
              "energyguide_audit": "OUT_OF_SCOPE_FOR_MONITOR", "status": status}
    (destination / "collection-summary.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    summary = os.getenv("GITHUB_STEP_SUMMARY")
    if summary:
        with Path(summary).open("a", encoding="utf-8") as stream:
            stream.write("## Monitor exact-SKU PDP collection\n\n")
            ...
            stream.write("FTC EnergyGuide collection, OCR, and comparison are outside the Monitor audit scope.\n\n")

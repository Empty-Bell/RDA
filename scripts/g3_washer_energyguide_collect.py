"""Retrieve and hash Washer PDP Support-declared EnergyGuide PDFs; no OCR or assessment."""
...
CONTRACT = "G3_WASHER_ENERGYGUIDE_RETRIEVAL_V1"
...
def load_documents(collection_root, collection_run_id):
    ...
    if summary.get("status") != "PASS" or str(summary.get("collection_run_id")) != str(collection_run_id):
        raise ValueError("Washer PDP collection artifact is not the requested successful run")
    ...
    for path in results:
        ...
        if not isinstance(sku, str) or not sku or sku in seen or result.get("status") != "VERIFIED_EXACT_IDENTITY":
            raise ValueError("Washer collection result is not a unique verified exact SKU")
        seen.add(sku)
        ...
        if not isinstance(user_agent, str) or "Chrome/" not in user_agent or "HeadlessChrome/" in user_agent:
            raise ValueError("Washer collection has no realistic desktop browser identity")
        if identity is None:
            identity = user_agent
        elif identity != user_agent:
            raise ValueError("Washer collection mixes browser identities")
        ...
        for index, doc in enumerate(docs):
            ...
            if (not parts or parts.scheme != "https" or not parts.hostname
                ...
                raise ValueError("Washer Support EnergyGuide URL must be HTTPS on samsung.com")

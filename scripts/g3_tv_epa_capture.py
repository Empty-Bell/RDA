"""Capture the complete current Samsung cohort from EPA's TV dataset, without matching."""
...
REQUIRED = {"pd_id", "brand_name", "model_number", "upc", "markets", "date_qualified",
            "reported_annual_energy_consumption_kwh", "diagonal_viewable_screen_size_inches",
            "power_consumption_in_on_mode_watts",
            "reported_on_mode_power_per_the_federal_test_procedure_watts"}


def fetch(url):
    request = Request(url, headers={"User-Agent": "RDA-G3-TV-EPA-Source/1.0", "Accept": "application/json"})
...
def capture(output, page_size=100):
    ...
    if status != 200 or "json" not in kind.lower():
        raise ValueError("EPA TV metadata unavailable")
    metadata = json.loads(meta_before_raw)
    if metadata.get("id") != DATASET or metadata.get("name") != "ENERGY STAR Certified Televisions":
        raise ValueError("EPA TV dataset identity changed")
    fields = [column.get("fieldName") for column in metadata.get("columns", [])]
    if not REQUIRED <= set(fields):
        raise ValueError("EPA TV dataset is missing required source fields")
    ...
    def query(params):
        ...
        if http_status != 200 or "json" not in content_type.lower():
            raise ValueError("EPA TV row query was not successful JSON")
        result = json.loads(body)
        if not isinstance(result, list) or any(not isinstance(item, dict) for item in result):
            raise ValueError("EPA TV query did not return an array of records")
        return result, url
    ...
    if len(count_rows) != 1 or not str(count_rows[0].get("row_count", "")).isdigit():
        raise ValueError("EPA TV Samsung count query malformed")
    expected = int(count_rows[0]["row_count"])
    if expected > 5000:
        raise ValueError("EPA TV Samsung cohort exceeds capture bound")
    pages, urls = [], []
    for offset in range(0, expected, page_size):
        page, url = query({"$where": BRAND_WHERE, "$select": select, "$order": ":id",
                           "$limit": str(page_size), "$offset": str(offset)})
        if not page:
            raise ValueError("EPA TV cohort ended before its count")
        pages.append(page)
        urls.append(url)
    rows = [row for page in pages for row in page]
    if len(rows) != expected:
        raise ValueError("EPA TV row pages do not match count")
    ids = [row.get("source_row_id") for row in rows]
    if any(not value for value in ids) or len(set(ids)) != len(ids):
        raise ValueError("EPA TV rows have missing or duplicated source identities")
    if any(not all(row.get(field) for field in ("pd_id", "brand_name", "model_number")) for row in rows):
        raise ValueError("EPA TV row is missing identity fields")
    count_after, _ = query({"$select": "count(*) as row_count", "$where": BRAND_WHERE})
    if int(count_after[0]["row_count"]) != expected:
        raise ValueError("EPA TV source row count changed during capture")
    meta_after_raw, kind, status = fetch(METADATA_URL)
    if status != 200 or "json" not in kind.lower():
        raise ValueError("EPA TV metadata refresh unavailable")
    ...
    if (metadata_after.get("id") != DATASET
        ...
        raise ValueError("EPA TV schema or source update timestamp changed during capture")
    ...
    report = {"contract": "G3_TV_EPA_CURRENT_SOURCE_V1", "dataset_id": DATASET,
              ...
              "scope": "Official EPA current Samsung TV dataset rows only; no model matching or certification assessment",
    ...
    if summary:
        with Path(summary).open("a", encoding="utf-8") as stream:
            stream.write("## TV EPA current source capture\n\n")

...
def pdp_facts(data, target, family='refrigerator'):
    ...
    return {'exact_sku': target,
            ...
            'active_display_size_raw': [x for x in fields if x['name'] == 'Active Display Size (HxV)'],
            ...
            'support_documents_raw': [] if specs_only else [
                {k: d.get(k) for k in ('name', 'type', 'url') if k in d}
                for d in support[0]['supports']],

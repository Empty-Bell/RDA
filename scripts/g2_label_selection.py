...
CAPACITY_GUBIC_OCR = re.compile(
    r"^\s*Capacity\s*:\s*(?P<amount>\d+(?:\.\d+)?)\s+Gubic\s+Feet\s*$", re.I
)


def select_capacity_strict(candidates: dict[str, Any]) -> dict[str, Any]:
    """Select one exact Capacity descriptor without correcting source text."""
    ...
    explicit = [
        ...
        if isinstance(entry, dict) and CAPACITY.fullmatch(str(entry.get("value_raw", "")))
    ]
    if len(explicit) != 1:
    ...
    return {
        ...
        "reason": "STRICT_UNIQUE_EXPLICIT_CAPACITY_DESCRIPTOR",
        "selection_basis": "STRICT_SOURCE_STRUCTURE",
...
def select_capacity(candidates: dict[str, Any], review: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    strict = select_capacity_strict(candidates)
    digest = candidates["pdf_sha256"]
    if review is None:
    ...
    if not isinstance(box, list) or len(box) != 4 or not all(isinstance(point, list) and len(point) == 2 for point in box):
        return unavailable("REVIEWED_CAPACITY_DETECTION_REQUIRED")
    if strict["observation"]["state"] != "VALUE":
        raw = review.get("capacity_text_raw")
        entries = candidates.get("capacity_candidates_raw", [])
        exact = [
            entry for entry in entries
            if isinstance(entry, dict) and entry.get("value_raw") == raw
        ]
        correction = CAPACITY_GUBIC_OCR.fullmatch(str(raw))
        visual_amount = review.get("capacity_visual_amount")
        if (
            len(exact) == 1
            and correction is not None
            and review.get("capacity_review")
            == "OCR_GUBIC_SUBSTITUTION_VISUALLY_CONFIRMED_CUBIC_FEET"
            and isinstance(visual_amount, (int, float))
            and math.isfinite(float(visual_amount))
            and float(visual_amount) == float(correction["amount"])
        ):
            return {
                "observation": {
                    "state": "VALUE",
                    "value": {
                        "amount": float(visual_amount),
                        "unit": "Cubic Feet",
                        "raw": raw,
                    },
                    "error": None,
                },
                "reason": "HASH_BOUND_VISUAL_REVIEW_CONFIRMED_GUBIC_TO_CUBIC_OCR_SUBSTITUTION",
                "selection_basis": "HASH_BOUND_VISUAL_REVIEW",
                "candidate": exact[0],
                "source_correction": {
                    "observed": "Gubic Feet",
                    "reviewed": "Cubic Feet",
        ...
        return strict
    raw = strict["candidate"]["value_raw"]
    if review.get("capacity_text_raw") != raw:
        return unavailable("REVIEWED_CAPACITY_TEXT_DOES_NOT_MATCH")
    return dict(
        strict,
        reason="HASH_BOUND_VISUAL_REVIEW_CONFIRMED_EXPLICIT_CAPACITY_DESCRIPTOR",
        selection_basis="HASH_BOUND_VISUAL_REVIEW",
    )


def select_annual_energy_strict(
    candidates: dict[str, Any], layout: dict[str, Any]
) -> dict[str, Any]:
    """Select one explicit kWh value uniquely linked to the annual-use caption."""
    digest = candidates.get("pdf_sha256")
    if not isinstance(digest, str) or not re.fullmatch(r"[0-9a-f]{64}", digest):
        raise ValueError("Candidate PDF provenance is invalid")
    ...
    matching = [
        entry for entry in energies
        if isinstance(entry, dict)
        and entry.get("unit_raw") == "kWh"
        and entry.get("value_raw") == proposal.get("value_raw")
    ]
    if len(matching) != 1:
        return unavailable("TEXT_AND_LAYOUT_CANDIDATES_DO_NOT_UNIQUELY_AGREE")
    annual = matching[0]
    raw_value = annual.get("value_raw")
    try:
    ...
    return {
        "observation": {
            ...
            "value": {
                "amount": amount,
                "unit": "kWh/year",
                "raw": annual.get("matched_text"),
            },
            "error": None,
        },
        "reason": "STRICT_UNIQUE_LAYOUT_MATCHED_KWH_WITH_ANNUAL_CAPTION",
        "selection_basis": "STRICT_SOURCE_STRUCTURE",
        "candidate": annual,
        "layout": associated[0],
    }
...
def select_annual_energy(candidates: dict[str, Any], layout: dict[str, Any], review: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    strict = select_annual_energy_strict(candidates, layout)
    digest = candidates["pdf_sha256"]
    if review is None:
        return unavailable("DOCUMENT_AND_PANEL_REVIEW_REQUIRED")
    if review.get("pdf_sha256") != digest:
    ...
    return dict(
        strict,
        reason="HASH_BOUND_VISUAL_REVIEW_CONFIRMED_EXPLICIT_KWH_WITH_ANNUAL_CAPTION_AND_LAYOUT",
        selection_basis="HASH_BOUND_VISUAL_REVIEW",
    )

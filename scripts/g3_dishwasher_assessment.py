...
def build(energy,numeric,model,out):
 ...
 for sku,row in zip(sorted(e),rows):
  nu,mo=n[sku],m[sku];mc=mo;nc=nu
  md.append(f"| {sku} | {row['display_outcome']} | {mc['pdp_vs_energyguide_model']} | {mc['pdp_vs_epa_model']} | {mc['energyguide_vs_epa_model']} | {nc['pdp_vs_energyguide_energy']} | {nc['energyguide_vs_epa_energy']} | {', '.join(x['issue_code'] for x in row['findings']) or '—'} |")
 d=Path(out);d.mkdir(parents=True,exist_ok=True);(d/'assessment.json').write_text(json.dumps(report,indent=2)+'\n');(d/'assessment.md').write_text('\n'.join(md)+'\n');summary=os.getenv('GITHUB_STEP_SUMMARY');Path(summary).write_text('\n'.join(md)+'\n',encoding='utf-8') if summary else None;print(json.dumps({'status':'PASS','sku_count':len(rows),'finding_count':len(all_findings),'counts':report['counts'],'by_sku':[{'exact_sku':sku,'outcome':row['display_outcome'],'comparisons':{'model':[m[sku][k] for k in ('pdp_vs_energyguide_model','pdp_vs_epa_model','energyguide_vs_epa_model')],'energy':[n[sku][k] for k in ('pdp_vs_energyguide_energy','energyguide_vs_epa_energy')]},'issue_codes':[f['issue_code'] for f in row['findings']]} for sku,row in zip(sorted(e),rows)]},sort_keys=True))

...
def build(energy,numeric,model,out):
 ...
 md=['# G3 Dishwasher final assessment','',f"SKUs: **{len(rows)}** | HIGH: **{report['counts']['HIGH']}** | MEDIUM: **{report['counts']['MEDIUM']}** | LOW: **{report['counts']['LOW']}** | PASS: **{report['counts']['PASS']}**",'', '| Exact SKU | Result | Findings |','|---|---|---|']
 md += [f"| {row['exact_sku']} | {row['display_outcome']} | {', '.join(x['issue_code'] for x in row['findings']) or '—'} |" for row in rows]
 d=Path(out);d.mkdir(parents=True,exist_ok=True);(d/'assessment.json').write_text(json.dumps(report,indent=2)+'\n');(d/'assessment.md').write_text('\n'.join(md)+'\n');summary=os.getenv('GITHUB_STEP_SUMMARY');Path(summary).write_text('\n'.join(md)+'\n',encoding='utf-8') if summary else None;print(json.dumps({'status':'PASS','sku_count':len(rows),'finding_count':len(all_findings),'counts':report['counts'],'by_sku':[{'exact_sku':x['exact_sku'],'outcome':x['display_outcome'],'issue_codes':[f['issue_code'] for f in x['findings']]} for x in rows]},sort_keys=True))

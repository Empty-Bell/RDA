"""Observe hash-verified Dryer EnergyGuide text/layout; do not select or assess values."""
...
CONTRACT = "G3_DRYER_ENERGYGUIDE_OBSERVATION_V1"
...
def observe_pdf(item, output, engine):
    ...
    if hashlib.sha256(raw).hexdigest() != digest or not raw.startswith(b"%PDF-"):
        raise ValueError("Dryer EnergyGuide source PDF hash or signature is invalid")
    target = output / "pdf" / digest
    target.mkdir(parents=True, exist_ok=True)
    pages, all_spans = [], []
    with pymupdf.open(stream=raw, filetype="pdf") as document:
        if not document.page_count:
            raise ValueError("Dryer EnergyGuide PDF has no pages")
...
def main():
    ...
    if len(source["sku_document_coverage"]) != source["sku_population_count"]:
        raise ValueError("Dryer label document coverage does not include the exact PDP population")
    ...
    report = {"contract": CONTRACT, "source": source,
              ...
              "scope": "Hash-verified Dryer EnergyGuide source text, layout, and unreadable-page OCR observations only",
    ...
    if summary_path:
        with Path(summary_path).open("a", encoding="utf-8") as stream:
            stream.write("## Dryer EnergyGuide observation\n\n")
            ...
            stream.write("OCR output is retained as raw observation only. Model, annual-energy, capacity selection and all compliance assessment remain disabled.\n\n")

...
def build(bundle: dict[str, Any], report: dict[str, Any], output: str | Path) -> dict[str, Any]:
    ...
    (destination / "index.html").write_text("""<!doctype html><meta charset=utf-8><title>Samsung Regulatory Audit</title><main><h1>Samsung Regulatory Audit</h1><p id=run></p><p id=summary></p><label>Filter <select id=severity><option value=''>All findings</option><option>HIGH</option><option>MEDIUM</option><option>LOW</option></select></label><table><thead><tr><th>SKU</th><th>Severity</th><th>Control</th><th>Finding</th></tr></thead><tbody id=findings></tbody></table></main><script>Promise.all(['data/run.json','data/summary.json','data/findings.json'].map(x=>fetch(x).then(x=>x.json()))).then(([r,s,f])=>{run.textContent='Run '+r.run_id;summary.textContent=`${s.finding_count} findings across ${s.affected_sku_count} SKUs`;let draw=()=>findings.innerHTML=f.filter(x=>!severity.value||x.severity==severity.value).map(x=>`<tr><td>${x.exact_sku}</td><td>${x.severity}</td><td>${x.control}</td><td>${x.title}</td></tr>`).join('');severity.onchange=draw;draw()})</script>""", encoding="utf-8")

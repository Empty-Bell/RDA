"""Summarize Washer label OCR/model/energy/capacity candidates without selecting or assessing."""
...
CONTRACT = "G3_WASHER_ENERGYGUIDE_RAW_CANDIDATE_REVIEW_V1"
...
def review(observation_root, observation_run_id, output):
    ...
    if summary.get("status") != "PASS" or str(summary.get("observation_run_id")) != str(observation_run_id):
        raise ValueError("Washer observation artifact is not the requested successful run")
    source = summary.get("source", {})
    sku_docs = source.get("sku_documents")
    if not isinstance(sku_docs, list) or len(sku_docs) != source.get("sku_document_count"):
        raise ValueError("Washer SKU-document provenance count is invalid")
    ...
    if not isinstance(sku_coverage, list) or len(sku_coverage) != source.get("sku_population_count"):
        raise ValueError("Washer per-SKU Support-document coverage is invalid")
    ...
    if (len(coverage_by_sku) != len(sku_coverage)
        ...
        raise ValueError("Washer per-SKU document counts disagree with exact-SKU PDF links")
    ...
    for link in sku_docs:
        ...
        if not isinstance(digest, str) or len(digest) != 64:
            raise ValueError("Washer SKU-document PDF hash is invalid")
    ...
    for item in summary.get("observations", []):
        ...
        if item.get("status") != "OBSERVED" or digest not in by_hash:
            raise ValueError("Washer observed PDF is not linked to an exact-SKU source document")
        relative = item.get("observation_path")
        if not isinstance(relative, str):
            raise ValueError("Washer observation path is missing")
        path = (root / relative).resolve()
        if not path.is_relative_to(root) or not path.is_file():
            raise ValueError("Washer observation path is missing or unsafe")
        observation = read_json(path)
        if observation.get("pdf_sha256") != digest:
            raise ValueError("Washer observation PDF hash does not match its source index")
    ...
    if summary_path:
        with Path(summary_path).open("a", encoding="utf-8") as stream:
            stream.write("## Washer EnergyGuide raw-candidate review\n\n")

"""Build same-run TV PDP/label/EPA model-identity comparisons without assessment."""
...
def main():
    ...
    for sku in population:
        ...
        label_model_state = ("MODEL_PATTERN_MATCHED" if label_matches else
                             "MODEL_PATTERN_UNMATCHED" if label_records else
                             "LABEL_NOT_ACCESSIBLE" if unreadable_labels else "NO_LABEL_DOCUMENT")
        epa_model_state = ("CURRENT_US_MODEL_MATCHED" if epa_us_matches else
                           "CURRENT_MODEL_MATCH_NON_US_ONLY" if epa_matches else "NO_CURRENT_MODEL_MATCH")
        ...
        table.append({"exact_sku": sku, "pdp_title_raw": listing.get("modelName"),
                      ...
                      "label_prefix_matches": label_matches,
                      "epa_current_model_matches": epa_matches,
                      "epa_match_count": len(epa_matches), "epa_us_match_count": len(epa_us_matches),
                      ...
                      "model_comparison": {"pdp": "EXACT_SKU_IDENTITY_VERIFIED" if pdp.get("status") == "VERIFIED_EXACT_IDENTITY" else "EXACT_SKU_IDENTITY_NOT_VERIFIED",
                                           "label": label_model_state, "epa_current": epa_model_state},
    ...
    report = {"contract": CONTRACT, "status": "PASS", "review_run_id": review.get("review_queue_run_id"),
              ...
              "model_match_counts": {
                  ...
                  "label_not_accessible": sum(row["model_comparison"]["label"] == "LABEL_NOT_ACCESSIBLE" for row in table),
                  "epa_current_us_model_matched": sum(row["model_comparison"]["epa_current"] == "CURRENT_US_MODEL_MATCHED" for row in table),
                  "epa_current_model_match_non_us_only": sum(row["model_comparison"]["epa_current"] == "CURRENT_MODEL_MATCH_NON_US_ONLY" for row in table),
                  "epa_current_no_model_match": sum(row["model_comparison"]["epa_current"] == "NO_CURRENT_MODEL_MATCH" for row in table)},
    ...
    if summary:
        with Path(summary).open("a", encoding="utf-8") as stream:
            stream.write("## TV PDP / label / EPA model identity audit\n\n")
            counts = report["model_match_counts"]
            stream.write(f"Exact SKUs: **{len(population)}**; readable EnergyGuide PDFs: **{review.get('unique_pdf_count')}**; EPA Samsung Current rows captured: **{len(epa_rows)}**; EPA current US model matches: **{counts['epa_current_us_model_matched']}**; label model pattern matches: **{counts['label_model_pattern_matched']}**; unreadable label HIGH candidates: **{report['finding_candidate_counts']['ENERGYGUIDE_FILE_NOT_READABLE_CANDIDATE']}**.\n\n")
            stream.write("TV energy values are intentionally excluded; this workflow compares model identity only.\n\n")
            stream.write("| Exact SKU | PDP identity | Label model pattern | EPA Current US model | EPA matched model pattern(s) |\n|---|---|---|---|---|\n")
            for row in table:
                label_cell = ("NOT ACCESSIBLE — HIGH finding" if row["model_comparison"]["label"] == "LABEL_NOT_ACCESSIBLE"
                              else ", ".join(row["label_model_patterns_raw"]) or "—")
                epa_models = ", ".join(f"{match['model_number_raw']} ({match['markets_raw']})" for match in row["epa_current_model_matches"]) or "—"
                cells = [row["exact_sku"], row["model_comparison"]["pdp"], label_cell,
                         row["model_comparison"]["epa_current"], epa_models]
            ...
            if unresolved_epa:
                ...
                for row in unresolved_epa:
                    normalized = "; ".join(f"{item['epa_model_pattern_raw']} ({item['markets_raw']})" for item in row["epa_normalized_pattern_matches_diagnostic_only"]) or "none"
    ...
    print(json.dumps({"status": report["status"], "population_count": len(population),
                      "epa_samsung_current_row_count": len(epa_rows),
                      "model_match_counts": report["model_match_counts"],

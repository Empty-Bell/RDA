"""Assess only Hood ENERGY STAR publication consistency using approved shared rules."""
import argparse
...
SOURCE_CONTRACT = "G3_HOOD_EPA_SOURCE_CANDIDATES_V1"
CONTRACT = "G3_HOOD_ENERGY_STAR_ASSESSMENT_V1"


def assess_record(row):
    sku = row.get("exact_sku")
    claim = row.get("energy_star_claim_sources_raw")
    candidates = row.get("epa_ventilating_fan_pattern_candidates")
    ...
    if not isinstance(candidates, list):
        raise ValueError("Hood EPA candidate list is missing")
    ...
    return {
        ...
        "epa_current_registration": {"state": registration,
                                      "ventilating_fan_model_pattern_candidates": candidates,
...
def build(candidates_path, output):
    ...
    if (source.get("contract") != SOURCE_CONTRACT
        ...
        raise ValueError("Hood source candidate artifact is not complete and validated")
    skus = [row.get("exact_sku") for row in rows]
    if not all(isinstance(sku, str) and sku for sku in skus) or len(set(skus)) != len(skus):
        raise ValueError("Hood candidates contain missing or duplicate exact SKUs")
    ...
    if step_summary:
        with Path(step_summary).open("a", encoding="utf-8") as stream:
            stream.write("## Hood ENERGY STAR publication consistency\n\n")

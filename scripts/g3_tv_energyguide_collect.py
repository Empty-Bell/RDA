...
def load_documents(collection_root, collection_run_id):
    ...
    if not isinstance(expected, int) or len(results) != expected:
        raise ValueError("TV collection artifact does not contain full PDP evidence")
    ...
    for path in results:
        ...
        if identity is None:
            ...
            raise ValueError("TV collection mixes browser identities")
        docs = result.get("pdp_facts_raw", {}).get("energyguide_documents")
        if not isinstance(docs, list):
            raise ValueError("TV Support document list is invalid")
        ...
        for index, doc in enumerate(docs):
            ...
            if (not parts or parts.scheme != "https" or not parts.hostname
                ...
                raise ValueError("TV Support EnergyGuide URL must be HTTPS on samsung.com")
...
def main():
    ...
    if summary_path:
        with Path(summary_path).open("a", encoding="utf-8") as stream:
            stream.write("## TV EnergyGuide PDF retrieval\n\n")

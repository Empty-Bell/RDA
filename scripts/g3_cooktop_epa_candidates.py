"""Join exact-SKU Cooktop claims to current EPA pattern candidates; no assessment."""
import argparse
...
COLLECTION_CONTRACT = "G3_COOKTOP_EXACT_SKU_PDP_V1"
EPA_CONTRACT = "G3_COOKTOP_EPA_CURRENT_SOURCE_V1"
REPORT_CONTRACT = "G3_COOKTOP_EPA_SOURCE_CANDIDATES_V1"
DATASET = "m6gi-ng33"
...
def load_collection(root, run_id):
    ...
    if (summary.get("status") != "PASS"
        ...
        raise ValueError("Cooktop PDP collection is not the requested successful contract/run")
    ...
    if not isinstance(expected, int) or len(products) != expected or len(paths) != expected:
        raise ValueError("Cooktop PDP collection is incomplete")
    product_skus = [row.get("exact_sku") for row in products]
    if len(set(product_skus)) != expected:
        raise ValueError("Cooktop product population has duplicate exact SKUs")
    claims = {}
    facts_by_sku = {}
    for path in paths:
        row = read_json(path)
        ...
        if sku not in set(product_skus) or row.get("status") != "VERIFIED_EXACT_IDENTITY":
            raise ValueError("Cooktop PDP result is failed, duplicate, or outside population")
        ...
        if hashlib.sha256(snapshot_raw).hexdigest() != row.get("snapshot_sha256"):
            raise ValueError("Cooktop PDP snapshot hash mismatch")
        snapshot = json.loads(snapshot_raw)
        if snapshot.get("target_sku") != sku:
            raise ValueError("Cooktop PDP snapshot lacks exact-SKU provenance")
        if sku in claims:
            raise ValueError("Cooktop PDP result contains duplicate SKU")
    ...
    if set(claims) != set(product_skus):
        raise ValueError("Cooktop PDP result set differs from the exact SKU population")
...
def load_epa(root, run_id):
    ...
    if (summary.get("status") != "PASS" or summary.get("contract") != EPA_CONTRACT
        ...
        raise ValueError("Cooktop EPA source capture identity/hash is invalid")
    rows = json.loads(raw)
    if not isinstance(rows, list) or len(rows) != summary.get("row_count"):
        raise ValueError("Cooktop EPA rows do not match capture summary")
    if any(not isinstance(row, dict) or not row.get("source_row_id")
        ...
        raise ValueError("Cooktop EPA row lacks source/model identity")
    return rows, summary
...
def build(collection_root, collection_run_id, epa_root, epa_run_id, output):
    ...
    for product in sorted(products, key=lambda row: row["exact_sku"]):
        ...
        cooktop_candidates, other_types = [], []
        for source in epa_rows:
            ...
            if str(source.get("product_type", "")).strip().casefold() == "cooktop":
                cooktop_candidates.append(row)
            else:
                other_types.append(row)
        candidate_count += len(cooktop_candidates)
        ...
        records.append({
            ...
            "cooktop_epa_pattern_candidates": cooktop_candidates,
            "same_pattern_non_cooktop_epa_rows": other_types,
    ...
    report = {
        ...
        "cooktop_pattern_candidate_row_count": candidate_count,
        "skus_with_cooktop_pattern_candidates": sum(bool(row["cooktop_epa_pattern_candidates"]) for row in records),
        "skus_without_cooktop_pattern_candidates": sum(not row["cooktop_epa_pattern_candidates"] for row in records),
        "skus_with_only_non_cooktop_product_candidates": sum(
            bool(row["same_pattern_non_cooktop_epa_rows"]) and not row["cooktop_epa_pattern_candidates"]
            for row in records),
        "same_pattern_non_cooktop_row_count": other_type_count,
        ...
        "scope": "Cooktop PDP claims and raw EPA electric-cooking model-pattern candidates only. EPA product_type and market are retained as evidence; no legal applicability conclusion is made.",
        "collection_contract": collection_summary["contract"],
        "records": records,
    }
    destination = Path(output)
    destination.mkdir(parents=True, exist_ok=True)
    (destination / "cooktop-epa-source-candidates.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    step_summary = os.getenv("GITHUB_STEP_SUMMARY")
    if step_summary:
        with Path(step_summary).open("a", encoding="utf-8") as stream:
            stream.write("## Cooktop EPA source candidates\n\n")
            stream.write(f"PDP exact SKUs: **{len(records)}**; EPA Samsung rows: **{len(epa_rows)}**; EPA Cooktop pattern candidates: **{candidate_count}**. No compliance decisions were run.\n\n")
            stream.write("| Exact SKU | PDP fuel facts | PLP flag | PDP logo | Spec certification | EPA Cooktop candidates | Other product-type matches |\n|---|---:|---|---:|---:|---:|---:|\n")
            for row in records:
                ...
                cells = [row["exact_sku"], len(row["pdp_product_facts_raw"]["fuel_or_cooktop_facts"]),
                         ...
                         len(row["cooktop_epa_pattern_candidates"]),
                         len(row["same_pattern_non_cooktop_epa_rows"])]
                stream.write("| " + " | ".join(str(value if value is not None else "(not observed)").replace("|", "\\|") for value in cells) + " |\n")
            stream.write("\nEPA candidates are evidence leads only. Cooktop applicability and every audit outcome remain NOT_EVALUATED.\n")
    examples = [{"sku": row["exact_sku"],
                 "pdp_fuel_facts": row["pdp_product_facts_raw"]["fuel_or_cooktop_facts"],
                 "cooktop_epa_patterns": [{"model": candidate["model_number_raw"],
                                         "markets": candidate["markets_raw"]}
                                        for candidate in row["cooktop_epa_pattern_candidates"]],
                 "other_epa_product_types": sorted({str(candidate["product_type_raw"])
                                                     for candidate in row["same_pattern_non_cooktop_epa_rows"]})}
                for row in records if row["cooktop_epa_pattern_candidates"]
                or row["same_pattern_non_cooktop_epa_rows"]]
    potential_electric_unmatched = []
    for row in records:
        if row["cooktop_epa_pattern_candidates"]:
            continue
    ...
    print(json.dumps({"status": report["status"], "source_validation": "PASS",
                      ...
                      "cooktop_pattern_candidate_rows": candidate_count,
                      "skus_with_cooktop_candidates": report["skus_with_cooktop_pattern_candidates"],
                      "skus_without_cooktop_candidates": report["skus_without_cooktop_pattern_candidates"],
                      "skus_with_only_other_product_type_candidates": report["skus_with_only_non_cooktop_product_candidates"],

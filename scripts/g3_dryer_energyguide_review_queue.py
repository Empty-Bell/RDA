"""Summarize Dryer label OCR/model/energy/capacity candidates without selecting or assessing."""
...
CONTRACT = "G3_DRYER_ENERGYGUIDE_RAW_CANDIDATE_REVIEW_V1"
...
def review(observation_root, observation_run_id, output):
    ...
    if summary.get("status") != "PASS" or str(summary.get("observation_run_id")) != str(observation_run_id):
        raise ValueError("Dryer observation artifact is not the requested successful run")
    source = summary.get("source", {})
    sku_docs = source.get("sku_documents")
    if not isinstance(sku_docs, list) or len(sku_docs) != source.get("sku_document_count"):
        raise ValueError("Dryer SKU-document provenance count is invalid")
    ...
    if not isinstance(sku_coverage, list) or len(sku_coverage) != source.get("sku_population_count"):
        raise ValueError("Dryer per-SKU Support-document coverage is invalid")
    ...
    for link in sku_docs:
        ...
        if not isinstance(digest, str) or len(digest) != 64:
            raise ValueError("Dryer SKU-document PDF hash is invalid")
    ...
    for item in summary.get("observations", []):
        ...
        if item.get("status") != "OBSERVED" or digest not in by_hash:
            raise ValueError("Dryer observed PDF is not linked to an exact-SKU source document")
        relative = item.get("observation_path")
        if not isinstance(relative, str):
            raise ValueError("Dryer observation path is missing")
        path = (root / relative).resolve()
        if not path.is_relative_to(root) or not path.is_file():
            raise ValueError("Dryer observation path is missing or unsafe")
        observation = read_json(path)
        if observation.get("pdf_sha256") != digest:
            raise ValueError("Dryer observation PDF hash does not match its source index")
        ...
        capacity_values = sorted({row.get("value_raw") for row in capacity_rows
                                  if row.get("value_raw") and re.search(r"drying\s+capacity\s*\(\s*cu\.?\s*ft\s*\)", row.get("value_raw", ""), re.I)})
    ...
    if summary_path:
        with Path(summary_path).open("a", encoding="utf-8") as stream:
            stream.write("## Dryer EnergyGuide raw-candidate review\n\n")

...
GROUPS = frozenset(('refrigerator', 'dishwasher', 'washer', 'tv', 'range', 'cooktop',
                    'dryer', 'hood', 'monitor', 'computer', 'tablet'))
ISSUES = frozenset(('ENERGYGUIDE_DOCUMENT_MISSING_CANDIDATE', 'ENERGYGUIDE_WRONG_DOCUMENT_CANDIDATE',
    'CRITICAL_WRONG_MODEL_LABEL_CANDIDATE', 'ENERGYGUIDE_OCR_MODEL_CHAR_CORRECTED',
    'ENERGYGUIDE_OCR_MODEL_CHAR_SUSPECT', 'ENERGYGUIDE_FILE_NOT_READABLE_CANDIDATE',
    'CRITICAL_ENERGY_STAR_ELIGIBILITY_CANDIDATE', 'SAMSUNG_ENERGY_STAR_SOURCE_CONFLICT',
    'ENERGY_STAR_VARIANT_IDENTITY_REVIEW', 'ENERGYGUIDE_KWH_NOT_EXTRACTED',
...
def timestamp(value):
    ...
    except ValueError as error: raise ContractError('Invalid timestamp') from error
    require(parsed.utcoffset() is not None, 'Timestamp requires timezone')


def record(cls, data):
    require(isinstance(data, dict), 'Record must be an object')
    require(set(data) == {f.name for f in fields(cls)}, 'Missing or unknown ' + cls.__name__ + ' fields')
    try: return cls(**data)
    except (TypeError, ValueError) as error: raise ContractError('Invalid ' + cls.__name__) from error
...
class Observation:
    ...
    def __post_init__(self):
        ...
        if self.state == ObservationState.VALUE:
            require(self.value is not None and self.error is None, 'Observed value requires non-null value/no error')
        elif self.state == ObservationState.ERROR:
            require(self.value is None and text(self.error), 'Error observation requires error/no value')
        else:
            require(self.value is None and self.error is None, 'Absent observation cannot contain a value/error')
...
class RunManifest:
    ...
    def __post_init__(self):
        ...
        require(text(self.run_id) and text(self.runner) and text(self.source_contract_version), 'Run identity missing')
        require(isinstance(self.git_sha, str) and bool(re.fullmatch(r'[a-f0-9]{40}', self.git_sha)), 'Invalid git SHA')
        require(sha(self.config_hash), 'Invalid config hash')
        require(text(self.python_version) and (self.playwright_version is None or text(self.playwright_version)), 'Invalid runtime version')
        require(type(self.assessment_enabled) is bool and not self.assessment_enabled, 'Assessment engine not enabled in Phase 1')
        require(self.rule_version is None, 'No approved rule version in this draft')
        object.__setattr__(self, 'overall_execution_status', PipelineStatus(self.overall_execution_status))
        timestamp(self.started_at)
        if self.completed_at is not None:
            timestamp(self.completed_at)
            require(datetime.fromisoformat(self.completed_at.replace('Z', '+00:00')) >=
                    datetime.fromisoformat(self.started_at.replace('Z', '+00:00')), 'Run completion precedes start')
...
class EvidenceRecord:
    ...
    def __post_init__(self):
        require(all(text(x) for x in (self.evidence_id, self.run_id, self.sku, self.parser_version, self.evidence_type)), 'Evidence identity missing')
        require(self.product_group in GROUPS and text(self.source_url) and self.source_url.startswith('https://'), 'Evidence source invalid')
        require(sha(self.sha256), 'Invalid evidence hash'); timestamp(self.captured_at)
        path = PurePosixPath(self.relative_path)
        require(text(self.relative_path) and not path.is_absolute() and '..' not in path.parts
                and '\\' not in self.relative_path and ':' not in self.relative_path, 'Unsafe evidence path')
...
def validate_bundle(data, *, synthetic_assessments=False):
    ...
    for raw in data['evidence']:
        ...
        require(item.run_id == manifest.run_id and item.product_group in products.get(item.sku, set()), 'Evidence run/SKU/group mismatch')
        evidence[item.evidence_id] = item
    def references(item):
        require(item.run_id == manifest.run_id and item.product_group in products.get(item.exact_sku, set()), 'Record run/SKU/group mismatch')
        require(isinstance(item.evidence_ids, list) and len(item.evidence_ids) == len(set(item.evidence_ids)), 'Invalid/duplicate evidence references')
    ...
    for raw in data['assessments']:
        item = record(AssessmentRecord, raw); references(item)
        require(text(item.assessment_id) and item.assessment_id not in seen, 'Invalid/duplicate assessment ID'); seen.add(item.assessment_id)
        require(item.regulatory_domain in ('FTC', 'EPA') and text(item.control_id), 'Invalid domain/control')
        require(text(item.reason), 'Assessment reason missing')
        require(item.automatic_final_legal_conclusion is False, 'Automatic legal conclusion prohibited')
        status = AssessmentStatus(item.assessment_status)
        if not synthetic_assessments:
            require(status == AssessmentStatus.NOT_EVALUATED and item.rule_id is None and item.rule_version is None
                    and item.issue_code is None and item.severity is None, 'Draft CLI cannot accept evaluated assessments')
        elif status == AssessmentStatus.FINDING:
            require(item.issue_code in ISSUES and item.severity in ('HIGH', 'MEDIUM', 'LOW') and item.evidence_ids,
                    'Synthetic finding requires existing issue/severity/evidence')
    if has_errors:
        require(manifest.overall_execution_status != PipelineStatus.SUCCESS, 'Source error cannot become execution SUCCESS')
        require(all(a['assessment_status'] == 'NOT_EVALUATED' for a in data['assessments']), 'Error bundle cannot claim evaluated status in draft')
    json.dumps(data, allow_nan=False)
    return data


def verify_evidence_files(data, root):
    ...
    for item in data['evidence']:
        ...
        require(path.is_relative_to(root) and path.is_file(), 'Evidence escapes root or is not a file')

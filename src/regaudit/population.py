...
def canonicalize_products(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for raw in records:
        ...
        runs.add(product.run_id)
        require(len(runs) <= 1, "Cannot merge different runs")
        require(
            isinstance(product.listings, list) and product.listings, "Listing provenance missing"
        )
        key = product.exact_sku
        if key not in products:
            products[key] = {"run_id": product.run_id, "exact_sku": key, "listings": []}
            provenance[key] = set()
        for listing in product.listings:
            ...
            if identity not in provenance[key]:
                products[key]["listings"].append(copy.deepcopy(listing))
                provenance[key].add(identity)

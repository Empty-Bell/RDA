...
현재 **Phase 0 진행 중 / G0 전체 미통과**, Phase 1–7 미착수다.
...
|---|---|---|
| **G0 종료 — 다음** | 최종 source matrix·계약/fixture coverage 대조, 미확인 경로와 의미 결정의 시점 고정, Phase 0 종료 기록 | 11군 source rows와 정상/빈 결과/페이지 경계/누락/중복/오류 계약 증거 확인. 기술 미확인 항목은 PASS 금지. 의미 결정 미해결 시 해당 경로를 명시적으로 보류 |
| **G1 골격 — 미착수** | typed schema, config, canonical SKU/provenance, run/evidence manifest, CLI, fixture CI; Python 3.11 호환성 | null/unknown/오류/다중 finding/중복 SKU/서로 다른 run 참조를 hosted CI에서 검증. D04/D08/D11/D12 관련 계약 확정 |
...
G0 이후 단계는 현재 AGENTS.md의 Phase 0 한정 범위가 확장된 뒤 착수한다.
이번 요청으로 후속 구현이나 공개 배포를 미리 실행하지 않는다.

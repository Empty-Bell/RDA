...
G2a full population-to-PDP identity coverage passed hosted run [35555806408](https://github.com/Empty-Bell/RDA/actions/runs/35555806408), commit `c0e46ca`: 75/75 verified, 0 failed, 0 unattempted. G2b source collection also has 75 parsed EnergyGuide facts, but reviewed field binding is incomplete: annual energy 8/75, capacity 9/75, raw model 69/75. The 75 facts map to 58 byte-distinct PDFs; the next gate is PDF-hash review coverage, not another live collection.

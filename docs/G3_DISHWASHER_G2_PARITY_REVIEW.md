...
| Capacity finding | Numeric PDP/EnergyGuide cubic-foot difference -> MEDIUM, zero tolerance | PDP and EPA place settings are comparable; EnergyGuide capacity is class text such as `Standard` | Do not copy the refrigerator capacity rule. Define a dishwasher-specific comparable field only if a numeric label field is available. |

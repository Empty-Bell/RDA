...
All eleven groups now have bounded source reconnaissance; see PHASE_STATUS.md and
...
Tablet uses /us/tablets/all-tablets/ and the hosted-verified selected-SKU/Specs-only
contract, with separate mAh, playback duration and main-display fields.
See TABLET_SOURCE_CONTRACT.md; its 50 listed SKUs represent one individual PDP sample.

Bounded raw claim/identity observations are now hosted-verified across all source legs;
